...
public class TimerArray<T> {
    ...
    public List<T> timeElapsed(Long millisecondsPast){
        ...
        for(Entry<T, Long> entry : timings.entrySet()){
    ...
    /**
     * Find objects that have elapsed, only usable with {@link TimerArray#addFuture(T, long)}
     * @param millisecondsPast
     * @return
     */
