 ...
 * Supported operators: + - / * ^
 * Variables  ($ prefixed) - Scalar
 * Functions  (@ prefixed) - Scalar
 * References (# prefixed) - Vector
 * @author James
 *
 */
public class InFixExpressionV2 {
	...
	public int getValue(VariableProvider varProvider,FunctionProvider funcProvider){
		...
		while(it.hasNext()){
			...
			int i = 0;
			switch(exp.charAt(0)){
			case '+':i = workingStack.pop(); workingStack.push(workingStack.pop() + i);break;
			case '-':i = workingStack.pop(); workingStack.push(workingStack.pop() - i);break;
			case '*':i = workingStack.pop(); workingStack.push(workingStack.pop() * i);break;
			case '/':i = workingStack.pop(); workingStack.push(workingStack.pop() / i);break;
			case '^':i = workingStack.pop(); workingStack.push((int) Math.pow(workingStack.pop() , i));break;
			...
			default:workingStack.push(Integer.parseInt(exp));break;
			}
		}
		if(workingStack.size() == 0){throw new IllegalArgumentException("Expression resulted in a empty stack!");}
